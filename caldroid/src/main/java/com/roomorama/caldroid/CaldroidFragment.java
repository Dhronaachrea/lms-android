...
public class CaldroidFragment extends DialogFragment {
    ...
    private OnCalenderButtonListener okListener;
    private OnCalenderButtonListener nowListener;
    private OnClickListener cancelListener;

    private CharSequence txtOk = "OK";
    private CharSequence txtCancel = "CANCEL";
    private CharSequence txtNow = "NOW";
    ...
    protected static int monthCurrent = -1;
    protected static int year = -1;
    public static int date = -1;
        private int onPageSlideYear = -1;
    private int onPageSlideMonth = -1;
    ...
    public void setNowClickListener(CharSequence text, final OnCalenderButtonListener listener) {
        txtNow = text;
        nowListener = listener;
    }

    public void setCancleClickListener(CharSequence text, OnClickListener listener) {
        txtCancel = text;
        cancelListener = listener;
    }

    public void setOkClickListener(CharSequence text, final OnCalenderButtonListener listener) {
        txtOk = text;
        okListener = listener;
    }
...
//        year = dateTime.getYear();
        ...
        onPageSlideMonth = dateTime.getMonth();
        onPageSlideYear = dateTime.getYear();
    ...
    private String days2[] = {"Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"};
    private String days3[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
            ...
            dateItemClickListener = new OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view,
                    ...
                    for (int j = 0; j < parent.getChildCount(); j++) {
                        if (((SquareTextView) parent.getChildAt(j)).getCustomStates().size() > 0 && ((Integer) ((SquareTextView) parent.getChildAt(j)).getCustomStates().get(0)).intValue() == CellView.STATE_TODAY) {
                            todayColorPos = j;
                        }
                        parent.getChildAt(j).setBackgroundColor(getResources().getColor(R.color.cal_bg_color));
                    }

                    if (dateTime.getMonth().intValue() == Calendar.getInstance().get(Calendar.MONTH) + 1) {
                        parent.getChildAt(todayColorPos).setBackgroundResource(R.drawable.cal_today_bg);
                    }

                    arrayList = ((SquareTextView) parent.getChildAt(position)).getCustomStates();
                    isNext = false;
                    isPrev = false;
                    for (Integer integer : arrayList) {
                        if (integer == CellView.STATE_PREV_MONTH) {
                            isPrev = true;
                        }
                        if (integer == CellView.STATE_NEXT_MONTH) {
                            isNext = true;
                        }
                    }
                    if (isPrev) {
                    ...
                    if (caldroidListener != null) {
                        ...
                        parent.getChildAt(position).setBackgroundResource(R.drawable.cal_selected_bg);
                        Date date = CalendarHelper
                                .convertDateTimeToDate(dateTime);
                        selectedDate = date;
                        //String[] formatedDate = dateFormat.format(selectedDate).split(" ");
                        String dateString = "" + days3[selectedDate.getDay()].toUpperCase(Locale.ENGLISH) + "," + " " + dateFormat.format(selectedDate);
                        setHeaderText(dateString);
                        year = dateTime.getYear();
                        CaldroidFragment.date = dateTime.getDay();
                        month = dateTime.getMonth();
...
//                        System.out.println("DAte " + dateString);
    ...
    protected void refreshMonthTitleTextView() {
        ...
        firstMonthTime.year = onPageSlideYear;
        firstMonthTime.month = onPageSlideMonth - 1;
    ...
    protected void retrieveInitialArgs() {
        ...
        if (args != null) {
            // Get month, year
            onPageSlideMonth = month = args.getInt(MONTH, -1);
            monthCurrent = args.getInt(MONTH, -1);
            onPageSlideYear = year = args.getInt(YEAR, -1);
            date = args.getInt(DAY, -1);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        headerText = (TextView) view.findViewById(R.id.header_date);
        now = (TextView) view.findViewById(R.id.now);
        cancle = (TextView) view.findViewById(R.id.cancel);
        ok = (TextView) view.findViewById(R.id.okay);
        ...
        if (nowListener != null) {
            now.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    nowListener.onClick(date, Integer.parseInt(formatedDate[0]), days3[date.getDay()], formatedDate[1], Integer.parseInt(formatedDate[2]));
                }
            });
        }
        if (cancelListener != null) {
            cancle.setOnClickListener(cancelListener);
        }
        if (okListener != null) {
            ok.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    String[] formatedDate = dateFormat.format(selectedDate).split(" ");
                    okListener.onClick(selectedDate, Integer.parseInt(formatedDate[0]), days3[selectedDate.getDay()], formatedDate[1], Integer.parseInt(formatedDate[2]));
                }
            });
        }
    }
    ...
    private void setupDateGridPages(View view) {
        ...
        adapter0.setAdapterDateTime(new DateTime(year, month, date, 0, 0, 0, 0));

        Date date = CalendarHelper.convertDateTimeToDate(new DateTime(year, month, this.date, 0, 0, 0, 0));
        selectedDate = date;
        String dateString = "" + days3[date.getDay()].toUpperCase(Locale.ENGLISH) + "," + " " + dateFormat.format(date);
        setHeaderText(dateString);
    ...
    protected ArrayList<String> getDaysOfWeek() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            list.add(fmt.format(date).toUpperCase(Locale.ENGLISH));
...
//                if ((currentDateTime.getDay().toString()).equalsIgnoreCase(CaldroidFragment.this.date+"") && (currentDateTime.getMonth().toString()).equalsIgnoreCase(CaldroidFragment.this.month+"") && (currentDateTime.getYear().toString()).equalsIgnoreCase(CaldroidFragment.this.year+""))
//                    currentAdapter.getView(position, new View(getActivity()), null).setBackgroundResource(R.drawable.cal_selected_bg);
