...
public class CaldroidGridAdapter extends BaseAdapter {
    ...
    private ArrayList<Integer> customViewStats;
    ...
    protected void setCustomResources(DateTime dateTime, View backgroundView,
        ...
        customViewStats = ((CellView) textView).getCustomStates();
        for (int i = 0; i < customViewStats.size(); i++) {
            Integer state = (Integer) customViewStats.get(i);
            if (state.intValue() == CellView.STATE_PREV_MONTH || state.intValue() == CellView.STATE_NEXT_MONTH) {
                ((CellView) textView).setTextColor(context.getResources().getColor(R.color.cal_text_dis_color));
                ((CellView) textView).setBackgroundColor(context.getResources().getColor(R.color.cal_bg_color));
            } else if (state.intValue() == CellView.STATE_TODAY) {
                ((CellView) textView).setTextColor(context.getResources().getColor(R.color.cal_text_color));
                ((CellView) textView).setBackgroundResource(R.drawable.cal_today_bg);
            }
        }
    }
    ...
    protected void customizeTextView(int position, CellView cellView) {
        ...
        if (dateTime.getMonth().intValue() < month) {
            cellView.addCustomState(CellView.STATE_PREV_MONTH);
        }
        if (dateTime.getMonth().intValue() > month) {
            cellView.addCustomState(CellView.STATE_NEXT_MONTH);
        }
...
//        if (date > 0 && (date+"").equalsIgnoreCase(dateTime.getDay().toString())) {
//            cellView.setBackgroundResource(R.drawable.cal_selected_bg);
//        }
        ...
        if ((dateTime.getDay().toString()).equalsIgnoreCase((CaldroidFragment.date+"")) && (dateTime.getMonth().toString()).equalsIgnoreCase(CaldroidFragment.month+"") && (dateTime.getYear().toString()).equalsIgnoreCase(CaldroidFragment.year+"")){
            cellView.setBackgroundResource(R.drawable.cal_selected_bg);
        }
