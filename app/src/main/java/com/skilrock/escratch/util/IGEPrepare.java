...
import com.skilrock.escratch.IGEException.TypeNotMatchException;
import com.skilrock.escratch.bean.IGEGamesData;
import com.skilrock.lms.ui.R;
import com.skilrock.utils.*;
...
public class IGEPrepare {
    ...
    double deviceHeight, deviceWidth, actionBarHeight, navigationMenuHeight = 0;
            ...
            if (spriteMping.equalsIgnoreCase("one-to-one")) {
                ...
                for (int i = 0; i < jsonArrayBackImgScrtchInfo.length(); i++) {
                    ...
                    if (type.equalsIgnoreCase("S")) {
                        IGEGamesData.BackImgScrtchInfo backImgScrtchInfo = new IGEGamesData.BackImgScrtchInfo(Math.round(xb * widthRatio), Math.round(yb * heightRatio), Math.round(xt * widthRatio), Math.round(yt * heightRatio), Math.round(scratchWidth * widthRatio), Math.round(scratchHeight * heightRatio), type, Math.round(wt * widthRatio), Math.round(ht * heightRatio));
                        backImgScrtchInfos.add(backImgScrtchInfo);
                    } else {
                        IGEGamesData.BackImgScrtchInfo backImgScrtchInfo = new IGEGamesData.BackImgScrtchInfo(Math.round(xb * widthRatio), Math.round(yb * heightRatio), Math.round(xt * widthRatio), Math.round(yt * heightRatio), Math.round(prizeWidth * widthRatio), Math.round(prizeHeight * heightRatio), type, Math.round(wt * widthRatio), Math.round(ht * heightRatio));
                        backImgScrtchInfos.add(backImgScrtchInfo);
                    }
                }
            }
...
//                if (resId > 0) {
//                    navigationMenuHeight = mContext.getResources().getDimensionPixelSize(resId);
//                }
//            }
//        } else {
//            navigationMenuHeight = 0;
//        }
        deviceHeight = displaymetrics.heightPixels - result - actionBarHeight - navigationMenuHeight;
        com.skilrock.utils.Utils.logPrint("deviceHeight :" + deviceHeight + "");
    ...
    public Bitmap getResultBitmap(Bitmap bitmap, IGEGamesData.PopUpPrzInfo przInfo, IGEGamesData.PopUpPlcingInfo popUpInfo, double przAmt) {
        ...
        Rect rect = new Rect(0, 0, (int) przInfo.getW(), (int) przInfo.getH() + 2);
        Typeface tanger = getTypeface(przInfo.getFontface());
        paint.setAntiAlias(true);
        paint.setSubpixelText(true);
        paint.setStyle(Paint.Style.FILL);
        paint.setColor(Color.WHITE);
        Typeface bold = Typeface.create(tanger, Typeface.BOLD);
        ...
        int numOfChars = paint.breakText((Config.CURRENCY_SYMBOL + " " + przAmt), true, width, null);
        if (przAmt > 0) {
            int start = ((Config.CURRENCY_SYMBOL + " " + przAmt).length() - numOfChars) / 2;
            canvas.drawText((Config.CURRENCY_SYMBOL + " " + przAmt), start, start + numOfChars, rect.exactCenterX(), rect.exactCenterY(), paint);
        }
        myCanvas.drawBitmap(bmp, (float) przInfo.getXp(), (float) przInfo.getYp(), null);

//        myCanvas.drawText(IGECommunication.CURRENCY_CODE + " " + przAmt, (float) przInfo.getXp(), (float) ((popUpInfo.getH() / 3) - przInfo.getYp()), paint);
    ...
    private Typeface getTypeface(String s) {
        if (s.contains("Comic")) {
            return Typeface.createFromAsset(mContext.getAssets(), "ComicSans.ttf");
        } else if (s.contains("Open")) {
            return Typeface.createFromAsset(mContext.getAssets(), "OPENSANS-BOLD.TTF");
        } else {
            return Typeface.createFromAsset(mContext.getAssets(), "ComicSans.ttf");
        }
    }
    ...
    public void setButtonStyle(TextView playForCash) {
        SpannableString styledString = new SpannableString("PLAY FOR CASH!!");
        Typeface typefaceThin = Typeface.createFromAsset(mContext.getAssets(), "ROBOTO-LIGHT.TTF");
        Typeface typefaceBold = Typeface.createFromAsset(mContext.getAssets(), "ROBOTO-REGULAR.TTF");
    ...
    public float convertPixelsToDp(float px, Context context) {
        ...
        if (!onTablet()) {
            ...
            return px / (metrics.densityDpi / 105f);
        }
    }

    private boolean onTablet() {
        int intScreenSize = mContext.getResources().getConfiguration().screenLayout
