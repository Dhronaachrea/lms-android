...
public class PathJSONParser {
	public static List<StepsLatLongs> stepsLatLongs;
		...
		try {
			...
			for (int i = 0; i < jRoutes.length(); i++) {
				...
				for (int j = 0; j < jLegs.length(); j++) {
					...
					stepsLatLongs = new ArrayList<StepsLatLongs>();
					/** Traversing all steps */
					for (int k = 0; k < jSteps.length(); k++) {
						StepsLatLongs stepsLatLong = new StepsLatLongs();
						JSONObject object = jSteps.getJSONObject(k);
						distance += ((JSONObject) object
								.getJSONObject("distance")).getInt("value");
						stepsLatLong.setInfo(object
						...
						LatLng latLng = new LatLng(
								...
								Double.parseDouble(((JSONObject) object
										.getJSONObject("end_location"))
										.getString("lng")));
						stepsLatLong.setLatLng(latLng);
						stepsLatLongs.add(stepsLatLong);
						String polyline = "";
